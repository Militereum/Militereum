...
procedure show(
  ...
  const logProc : TLogProc);
begin
  if whitelisted(TFrmUnlock) or whitelisted(TFrmUnlock, token) then
  begin
    callback(True, False);
    EXIT;
  end;
  ...
  begin
    const frmUnlock = TFrmUnlock.Create(chain, tx, callback, logProc);
...
procedure TFrmUnlock.SetAction(const value: TTokenAction);
begin
  lblTitle.Text := System.SysUtils.Format(lblTitle.Text, [ActionText[value]]);
end;

procedure TFrmUnlock.SetToken(const value: TAddress);
begin
  FToken := value;
...
function TFrmUnlock.Bypass: TBypass;
begin
  Result := TBypass.Create('token', procedure
  begin
    whitelist(TFrmUnlock, FToken);
  end);
end;
